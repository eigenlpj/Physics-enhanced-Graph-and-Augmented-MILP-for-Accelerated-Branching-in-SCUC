    ...
    def branchexeclp(self, allowaddcons):
        ...
        # custom policy branching
            ...
            if len(candidate_vars) == 1:
                ...
                start_time = time.perf_counter()
                GNNPolicy_Pinfo_list =  ['gcnn_aug', 'IGNN_NoAMILP', 'IGNN_5k', 'PGNNwithDepth']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--gnn-model',
        nargs='+',
        choices=['gcnn', 'gcnn_0.1', 'gcnn_huake', 'IGNN_NoPgraph', 'IGNN_NoAMILP', 'IGNN_5k', 'gcnn_aug', 'PGNNwithDepth']
    )
    ...
    if args.problem == 'case1888':

        ##                    F4
        # user_selected_ids = [2431 ]

        #                       F1    F2    F3    F4    F5    F6    F7    F8    F9    F10
        user_selected_ids =    [2401, 2411, 2421, 2431, 2441, 2451, 2461, 2471, 2481, 2491 ]
        #                      F11   F12   F13   F14   F15   F16   F17   F18   F19   F20
        user_selected_ids2 =   [2501, 2511, 2521, 2531, 2541, 2551, 2561, 2571, 2581, 2591 ]
        #                       F21   F22   F23   F24   F25   F26   F27   F28   F29   F30
        user_selected_ids3 =   [2405, 2415, 2425, 2435, 2445, 2455, 2465, 2475, 2485, 2495 ]
        instances += [{'type': f'{args.gpu}(-1:cpu,others:gpu)', 'path': f"D:/LiJiamigFile/CAMBranch-ljmdata/data/instances/case1888-v1/test_milp/case1888_{i}/case1888_{i}.lp"} for i in user_selected_ids]
        instances += [{'type': f'{args.gpu}(-1:cpu,others:gpu)', 'path': f"D:/LiJiamigFile/CAMBranch-ljmdata/data/instances/case1888-v1/test_milp/case1888_{i}/case1888_{i}.lp"} for i in user_selected_ids2]
        instances += [{'type': f'{args.gpu}(-1:cpu,others:gpu)', 'path': f"D:/LiJiamigFile/CAMBranch-ljmdata/data/instances/case1888-v1/test_milp/case1888_{i}/case1888_{i}.lp"} for i in user_selected_ids3]
            ...
            if policy['name'] not in loaded_models:
                ...
                if policy['name'] == 'gcnn' or policy['name'] == 'gcnn_0.1':
                    ...
                    model.load_state_dict(
                        torch.load(
                            r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_0.1_gasse\0\gcnn_0.1_gasse.pkl",
                            map_location=device,
                            weights_only=True)
                    )
                    print(
                        r"load model 1: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_0.1_gasse\0\gcnn_0.1_gasse.pkl")

                    model = model.to(device)
                    model.eval()
                    # print(f"load model 1: gcnn_0.1_gasse.pkl to be training...")
                    # exit(1)
                    ...
                    model.load_state_dict(
                        torch.load(
                            r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_0.1_huake\0\gcnn_0.1_huake.pkl",
                            map_location=device,
                            weights_only=True)
                    )
                    print(
                        r"load model 2: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_0.1_huake\0\gcnn_0.1_huake.pkl")
                    model = model.to(device)
                    model.eval()
                    ...
                    model.load_state_dict(
                        torch.load(
                            r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\NoPgraph\gcnn_aug_1.pkl",
                            map_location=device,
                            weights_only=True)
                    )
                    print(
                        r"load model 3: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\NoPgraph\gcnn_aug_1.pkl")
                    model = model.to(device)
                    model.eval()
                    ...
                    model.load_state_dict(
                        torch.load(
                            r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\PGNN\gcnn_aug_1.pkl",
                            map_location=device,
                            weights_only=True)
                    )
                    print(
                        r"load model 6: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\PGNN\gcnn_aug_1.pkl")

                    model = model.to(device)
                    model.eval()
                elif policy['name'] == 'PGNNwithDepth':
                    model = GNNPolicy_Pinfo(cl=True)

                    # case1888
                    model.load_state_dict(
                        torch.load(
                            r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\PGNNwithDepth\gcnn_aug_1.pkl",
                            map_location=device,
                            weights_only=True)
                    )
                    print(
                        r"load model 7: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\case1888\gcnn_aug\0\0.05_0.01\PGNNwithDepth\gcnn_aug_1.pkl")
