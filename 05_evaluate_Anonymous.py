    ...
    def branchexeclp(self, allowaddcons):
        ...
        # custom policy branching
            ...
            if len(candidate_vars) == 1:
                ...
                start_time = time.perf_counter()
                GNNPolicy_Pinfo_list =  ['gcnn_aug', 'IGNN_NoAMILP', 'IGNN_5k']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--gnn-model',
        nargs='+',
        choices=['gcnn', 'gcnn_0.1', 'gcnn_huake', 'IGNN_NoPgraph', 'IGNN_NoAMILP', 'IGNN_5k', 'gcnn_aug']
    )
    ...
    if args.problem == '24GX':

        #                      A8
        # user_selected_ids_1 = [2505]

        # #                       A1    A2    A3    A4    A5    A6    A7    A8    A9
        # user_selected_ids_1 = [2401, 2402, 2403, 2404, 2471, 2475, 2502, 2505, 2571]
        #                      A11    A12   A13   A14   A15   A16   A17   A18   A19   A20
        # user_selected_ids_1 = [2450, 2460, 2470, 2480, 2490, 2500, 2510, 2520, 2530, 2540]
        #                      A21    A22   A23   A24   A25   A26   A27   A28   A29   A30
        user_selected_ids_1 = [2459, 2469, 2479, 2489, 2499, 2509, 2519, 2529, 2539, 2549]
        instances += [{'type': f'{args.gpu}(-1:cpu,others:gpu)', 'path': f"D:/LiJiamigFile/CAMBranch-ljmdata/data/instances/24GX-v4/test_milp/24GX_{i}/24GX_{i}.lp"} for i in user_selected_ids_1]
        # #                      A10
        # user_selected_ids_2 = [1]
        # instances += [{'type': f'{args.gpu}(-1:cpu,others:gpu)', 'path': f"D:/LiJiamigFile/CAMBranch-ljmdata/data/instances/24GX-v4-loadtest/24GX_{i}/24GX_{i}.lp"} for i in user_selected_ids_2]
            ...
            if policy['name'] not in loaded_models:
                ...
                if policy['name'] == 'gcnn' or policy['name'] == 'gcnn_0.1':
                    ...
                    model.load_state_dict(
                        torch.load(r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_0.1_gasse\gcnn_0.1_gasse.pkl",
                                   map_location=device,
                                   weights_only=True)
                    )
                    print(r"load model 1: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_0.1_gasse\gcnn_0.1_gasse.pkl")
                    ...
                    # 24GX
                    model.load_state_dict(
                        torch.load(r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_0.1_huake\gcnn_0.1_huake.pkl",
                                   map_location=device,
                                   weights_only=True)
                    )
                    print(r"load model 2: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_0.1_huake\gcnn_0.1_huake.pkl")
                    ...
                    # 24GX
                    model.load_state_dict(
                        torch.load(r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_aug\NoPgraph\gcnn_aug_1.pkl",
                                   map_location=device,
                                   weights_only=True)
                    )
                    print(r"load model 3: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_aug\NoPgraph\gcnn_aug_1.pkl")
                    model = model.to(device)
                    model.eval()
                    ...
                    # 24GX
                    model.load_state_dict(
                        torch.load(r"D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_aug\PGNN\gcnn_aug_1.pkl",
                                   map_location=device,
                                   weights_only=True)
                    )
                    print(r"load model 4: D:\LiJiamigFile\CAMBranch-ljmdata\KIDA_log\24GX\gcnn_aug\PGNN\gcnn_aug_1.pkl")
                    model = model.to(device)
                    model.eval()
                else:
