...
if __name__ == "__main__":
    ...
    for y in range(BEGIN_NO + 1, BEGIN_NO + NUM_FOLDERS + 1):
        ...
        # 构建文件路径
        system_load_path = os.path.join(new_folder_path, f"3-{caseName}-系统负荷.csv")
        bus_load_path = os.path.join(new_folder_path, f"2-{caseName}-母线负荷.csv")
        generator_data_path = os.path.join(new_folder_path, f"5-{caseName}-机组数据.csv")
        line_data_path = os.path.join(new_folder_path, f"4-{caseName}-线路参数.csv")
        ...
        # 处理 5-{caseName}-机组数据.csv：先处理原目标列
        ...
        if y >= G_RANGE_BEGIN_FOLDER:
            ...
            if valid_units_to_delete:
                ...
                print(f"  已删除机组数据的第 {', '.join(deleted_lines)} 行（原始索引 {valid_units_to_delete}）")
            else:
                print("  无有效机组可删除")
        ...
        origin_rate = bus_load_df.iloc[:, 1] / sum_demand  # 第2列各元素占 原始系统负荷 的比例
        total_adjusted = sum(c * r for c, r in zip(c_values, origin_rate))

        # 缩放 2-{caseName}-母线负荷.csv 的第二列
        bus_load_df.iloc[:, 1] *= (max_demand / sum_demand)
        bus_load_df.to_csv(bus_load_path, index=False)

        # 使用 c_values * origin_rate[i] / total_adjusted 作为后来的各母线负荷占比，调整每行第二列
