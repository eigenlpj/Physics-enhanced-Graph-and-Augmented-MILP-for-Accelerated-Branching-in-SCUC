...
def getLinedata(filename, busnum, slack, output_PTDF_path):
    ...
    if os.path.isfile(output_PTDF_path) is False:
        ...
        row_names = ['Line No.' + str(i + 1) for i in range(linenum)]
...
if __name__ == '__main__':
    ...
    PlinelimitCount, linelist = getLinedata(Dir1 + "\\4-" + caseName + "-linedata.csv", len(buslist), int(slackBus),
    ...
    busHydroNewlist = getbusHydroNewdata(Dir1 + "\\1-" + caseName + "-busname.csv",
                                         Dir1 + "\\3-" + caseName + "-renewableEnergy.csv",T)
